...
const App = () => {
  const [role, setRole] = useState<'landing' | 'farmer' | 'buyer' | 'logistics'>('landing');

  return (
    <div className="min-h-screen bg-background font-sans selection:bg-primary selection:text-primary-foreground">
      <Layout role={role} onHome={() => setRole('landing')}>
              ...
              >
                <div className="relative rounded-3xl overflow-hidden h-[500px] mb-16 flex items-center justify-center text-center p-8">
                  ...
                  <div className="relative z-10 max-w-3xl">
                    ...
                    >
                      <Button size="lg" onClick={() => setRole('farmer')} className="bg-white text-primary hover:bg-white/90 font-bold h-14 px-8 text-lg gap-2">
                        <Tractor /> I'm a Farmer
                      </Button>
                      <Button size="lg" onClick={() => setRole('buyer')} className="bg-primary text-primary-foreground hover:bg-primary/90 font-bold h-14 px-8 text-lg gap-2">
                        <ShoppingBag /> I'm a Buyer
                      </Button>
                      <Button size="lg" variant="secondary" onClick={() => setRole('logistics')} className="font-bold h-14 px-8 text-lg gap-2">
                        <Truck /> Logistics
                      </Button>
                    </motion.div>
                  </div>
